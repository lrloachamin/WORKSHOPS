...
public static ArrayList<Product> createFileWorker(ArrayList<Product> worker, File file) {
int id;
String name;
 float cost;
 int porcentaje;
 float pvp;
		try {
			...
			try (FileReader archedad = new FileReader(file)) {
				...
				while ((read != null)) {
					if (read.contains(";")) {
						id=  Integer.parseInt(read.split(";")[0]);
						name =read.split(";")[1];
						cost =Float.parseFloat(read.split(";")[2]);
						porcentaje =  Integer.parseInt(read.split(";")[3]);
                                                pvp=Float.parseFloat(read.split(";")[4]);
						worker.add(new Product(id,name,cost,porcentaje,pvp));
					}
					read = br.readLine();
				}
			}
    ...
    public static void FileWorker(File file, ArrayList<Product> worker) {
		String letter = "";
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(file));
			for (int k = 0; k < worker.size(); k++) {
				letter = worker.get(k).getId() + ";" +worker.get(k).getName() + ";"
						+worker.get(k).getCost()+ ";" + worker.get(k).getPorcentaje()+";"+worker.get(k).getPvp();
				bw.write(letter);
				bw.newLine();
			}
			bw.close();
