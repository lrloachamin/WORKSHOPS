...
public class FileManager {
    ...
    public static ArrayList<Calculate> createFileWorker(ArrayList<Calculate> worker, File file) {
			...
			try (FileReader archedad = new FileReader(file)) {
				...
				while ((read != null)) {
					if (read.contains(";")) {
						...
						Vo =Float.parseFloat(read.split(";")[0]);
						
						angulo = Float.parseFloat(read.split(";")[1]);
                                                max=Float.parseFloat(read.split(";")[1]);
						worker.add(new Calculate(Vo,angulo,max));
					}
					read = br.readLine();
				}
			}
    ...
    public static void FileWorker(File file, ArrayList<Calculate> worker) {
		String letter = "";
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(file));
			for (int k = 0; k < worker.size(); k++) {
				letter = worker.get(k).getVo() + ";" +worker.get(k).getAngulo()+ ";"
						+worker.get(k).getMax();
				bw.write(letter);
				bw.newLine();
			}
